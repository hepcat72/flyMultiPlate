%% setup file structure and camera
close all;
clear refStack;
clear refImage;

experimentLength=259200;               % Length of the trial in seconds
W=1328;                              % Dimensions of the image in Width & Height
H=1048;
...
% first have the user click the corners of the plate (6) and return the
% positions of the wells

foundWells.coords = nan;
while all(isnan(foundWells.coords))
    try
        im=peekdata(vid,1);                     % acquire image from camera
        % im=squeeze(im(:,:,imagingLayer));                  % Extracting the red channel (works the best)
        refStack=double(im);
        out=[];
        foundWells=find96WellPlateUserClick_HighRes1plate(im,1);
        
        button = questdlg('Keep this grid of wells?','Grid Found?','keep','try again','keep');
        switch button
            case 'try again'
